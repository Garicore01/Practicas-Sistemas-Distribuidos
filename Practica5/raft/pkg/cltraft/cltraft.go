...
func main(){
	...
	op3:= raft.TipoOperacion{Operacion: "escribir", Clave: "x", Valor: "8"}
	op4:= raft.TipoOperacion{Operacion: "leer", Clave: "x", Valor: ""}
	log.Println("Buscando lider")
	

	var err error
	err = nodos[0].CallTimeout("NodoRaft.SometerOperacionRaft", op1, &reply, 20000 * time.Millisecond)
	...
	if !reply.EsLider{
		...
		err = nodos[reply.IdLider].CallTimeout("NodoRaft.SometerOperacionRaft", op1, &reply, 20000 * time.Millisecond)
	...
	err = nodos[reply.IdLider].CallTimeout("NodoRaft.SometerOperacionRaft", op2, &reply, 20000 * time.Millisecond)
	...
	err = nodos[reply.IdLider].CallTimeout("NodoRaft.SometerOperacionRaft", op3, &reply, 20000 * time.Millisecond)
	check.CheckError(err, "Error en SometerOperacion 3")
	log.Println("Operacion3 sometida a ", reply.IdLider)
	log.Println("Resultado de la operacion3: ", reply.ValorADevolver)
	
	err = nodos[reply.IdLider].CallTimeout("NodoRaft.SometerOperacionRaft", op4, &reply, 20000 * time.Millisecond)
	check.CheckError(err, "Error en SometerOperacion 4")
	log.Println("Operacion4 sometida a ", reply.IdLider)
	log.Println("Resultado de la operacion3: ", reply.ValorADevolver)
